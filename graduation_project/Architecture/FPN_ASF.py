...
from graduation_project.Architecture.Feature_Attention import ScaleFeatureSelection


class FeaturePyramid_ASF(nn.Module):
    def __init__(self, in_channels=None):
        super(FeaturePyramid_ASF, self).__init__()
        ...
        self.concat_attention = ScaleFeatureSelection(256, 256 // 4,
                                                      attention_type='scale_spatial')
        self.binarize = nn.Sequential(
            nn.Conv2d(256, 256 // 4, 3, bias=False, padding=1),
            nn.BatchNorm2d(256 // 4),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(256 // 4, 256 // 4, 2, 2),
            nn.BatchNorm2d(256 // 4),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(256 // 4, 1, 2, 2),
            nn.Sigmoid())
...
if __name__ == "__main__":
    # 创建特征金字塔模型实例
    fpn1 = FeaturePyramid_ASF()

    # 使用示例输入进行测试
    input5 = torch.randn(1, 480, 20, 20)
    input4 = torch.randn(1, 56, 40, 40)
